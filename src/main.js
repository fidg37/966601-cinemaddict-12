...
renderTemplate({container: SiteElements.MAIN, template: createSortTemplate()});
renderTemplate({container: SiteElements.MAIN, template: createContentFieldTemplate()});
...
for (let i = 0; i < MAX_FILMS_PER_STEP; i++) {
  renderTemplate({container: mainFilms, template: createFilmCardTemplate(films[i])});
}
